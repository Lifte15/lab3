...
if (isset($_POST['user_otp']) && isset($_POST['emails'])) {
    ...
    if (empty($user_otp) && empty($email)) {
        header("Location: successemail.php?error=Email and OTP is required");
        exit();
    } else if (empty($email)) {
        header("Location: successemail.php?error=Email is required");
        exit();
    } else if (empty($user_otp)) {
        header("Location: successemail.php?error=OTP is required");
        exit();
    }
    else {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                if ($row['is_verified'] == 0 && $user_otp === $v_code) {
                    ...
                    if (mysqli_query($conn, $update)) {
                        ...
                        header("Location: successemail.php?error=Unable to update verification status");
                        exit();
                    }
                } else {
                    // Redirect to the success form with error message
                    header("Location: successemail.php?error=Invalid OTP");
                    exit();
                }
            } else {
                // Redirect to the success form with error message
                header("Location: successemail.php?error=User not found");
                exit();
            }
        } else {
            // Redirect to the success form with error message
            header("Location: successemail.php?error=Database query failed");
            exit();
        }
    }
} else {
